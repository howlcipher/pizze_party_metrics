...
def analyze_metrics(input_path: str, output_path: str) -> None:
    ...
    df_clean.loc[:, 'focus_meeting_ratio'] = df_clean['focus_hours'] / \
        df_clean['meeting_overhead'].replace(0, np.nan)
    ...
    if 'burnout_risk_score' in df_clean.columns:
        burnout_profile = (df_clean.groupby(['work_setup_category', 'industry'])[
            'burnout_risk_score'] .mean() .round(4) .reset_index())
    else:
    ...
    if 'interruption_frequency' in df_clean.columns and 'focus_hours' in df_clean.columns:
        ...
        df_clean['true_productive_hours'] = (
            df_clean['focus_hours'] - (df_clean['interruption_frequency'] * 0.33)).clip(lower=0)
    ...
    insights = {
        "industry_profile": industry_profile.to_dict(
            orient="records"),
        ...
        "burnout_risk_profile": burnout_profile.to_dict(
            orient="records"),
        "true_productivity_profile": true_productivity_profile.to_dict(
            orient="records")}
...
if __name__ == "__main__":
    analyze_metrics("src/data/pizza_metrics.json",
                    "src/data/advanced_collaboration_insights.json")
